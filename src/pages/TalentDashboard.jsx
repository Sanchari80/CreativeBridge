...
// Actor & Dancer: Drive link only (videos too large for Cloudinary free tier)
// Singer:         Both file upload (audio/small video) AND Drive link
// Painter:        File upload only (images, copyright-protected)
const CONFIG = {
  ...
  Painter: { folder:'artworks', label:'Artworks', icon:'🎨', accept:'image/*',          resourceType:'auto', maxMB:10,  hint:'JPG, PNG — সম্পূর্ণ Copyright protected',                              mode:'fileOnly'  },
...
// ── Detect Drive links → embed URL (mirrors CommonDashboard logic) ──
...
const TalentDashboard = () => {
  ...
  // Singer only: toggle between file upload and Drive link
  ...
  const handleUpload = async () => {
    ...
    // Singer file mode
    if (cfg.mode === 'both' && uploadMode === 'file' && !file) return alert('File select করুন!');
    // Singer link mode
    if (cfg.mode === 'both' && uploadMode === 'link') {
      if (!driveLink.trim())                      return alert('Drive link দিন!');
      if (!driveLink.trim().startsWith('http'))   return alert('সঠিক link দিন!');
    }
    // Painter
    if (cfg.mode === 'fileOnly' && !file) return alert('File select করুন!');
    ...
    try {
      ...
      if (cfg.mode === 'linkOnly') {
        ...
        mediaType = undefined;
        setProgress(100);
      } else if (cfg.mode === 'both' && uploadMode === 'link') {
        // Singer: Drive link chosen
        finalUrl  = driveLink.trim();
        mediaType = linkMediaType;
        setProgress(100);
      } else {
        // Singer file upload OR Painter
      ...
      setFile(null); setDriveLink(''); setForm({ title:'', extra:'' });
      setShowUpload(false); setUploadMode('file');
        ...
        <div style={uploadCard}>
          ...
          {extraCfg && (
            <input placeholder={extraCfg.placeholder} value={form.extra}
              onChange={e => setForm(f => ({ ...f, extra: e.target.value }))} style={inp}/>
          )}
          ...
          {cfg.mode === 'linkOnly' && (
            <div style={linkBox}>
              ...
              <div style={{ background:'#f0f4ff', borderRadius:10, padding:'10px 12px', fontSize:12, color:'#4834d4' }}>
                <strong>How to get your Drive link:</strong><br/>
                Google Drive তে video upload করো → Right click → Share → "Anyone with the link" → Link copy করো → এখানে paste করো
              </div>
            </div>
          )}

          {/* ── SINGER: file OR Drive link toggle ── */}
          {cfg.mode === 'both' && (
            <>
              <div style={{ display:'flex', gap:8, marginBottom:12 }}>
                <button onClick={() => setUploadMode('file')} style={{ ...modeBtn, ...(uploadMode==='file' ? modeBtnActive : {}) }}>
                  📤 Upload File
                </button>
                <button onClick={() => setUploadMode('link')} style={{ ...modeBtn, ...(uploadMode==='link' ? modeBtnActive : {}) }}>
                  🔗 Drive Link
                </button>
              </div>
                ...
                <>
                  ...
                  <input ref={fileInputRef} type="file" accept={cfg.accept}
                    onChange={e => {
                      ...
                      if (!f) return;
                      if (f.size > cfg.maxMB * 1024 * 1024) { alert(`Max ${cfg.maxMB}MB. Use Drive Link for bigger files.`); e.target.value=''; return; }
                      setFile(f);
                    }}
                    style={{ display:'none' }}/>
                </>
              ...
              {uploadMode === 'link' && (
                <div style={linkBox}>
                  <p style={linkLabel}>🔗 Google Drive Link *</p>
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    <button onClick={() => setLinkMediaType('audio')} style={{ ...modeBtn, flex:1, ...(linkMediaType==='audio' ? modeBtnActive : {}) }}>🎵 Audio</button>
                    <button onClick={() => setLinkMediaType('video')} style={{ ...modeBtn, flex:1, ...(linkMediaType==='video' ? modeBtnActive : {}) }}>🎬 Video</button>
                  </div>
                </div>
              )}
            </>
            ...
            <>
              ...
              <input ref={fileInputRef} type="file" accept={cfg.accept}
                onChange={e => {
                  const f = e.target.files[0];
                  if (!f) return;
                  if (f.size > cfg.maxMB * 1024 * 1024) { alert(`Max ${cfg.maxMB}MB`); e.target.value=''; return; }
                  setFile(f);
                }}
                style={{ display:'none' }}/>
              <div style={noticeBox}>🔒 তোমার artwork copyright-protected হবে। কেউ download বা screenshot করতে পারবে না।</div>
            </>
          )}
          ...
          <button onClick={handleUpload} disabled={uploading}
            style={{ ...uploadBtn, width:'100%', padding:'13px', opacity: uploading ? 0.7 : 1 }}>
            {uploading
              ? (cfg.mode==='linkOnly' || uploadMode==='link') ? 'Saving...' : `Uploading... ${progress}%`
              : '🚀 Publish'}
          </button>
        </div>
      )}
                ...
                <div key={work.id} style={workCard}>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:'8px', flexShrink:0 }}>
                      <button onClick={() => handleCopyLink(work.id)} style={copyBtn}>
                  ...
                  {role === 'Singer' && work.fileUrl && (
                    fromDrive ? (
                      <iframe src={driveEmbedUrl(work.fileUrl)}
                        style={{ width:'100%', height: work.mediaType==='video' ? 200 : 80, border:'none', borderRadius:'10px', marginTop:'10px' }}
                        allow="autoplay" title={work.title}/>
                  ...
                  {/* Actor / Dancer — Drive link player */}
                  {(role === 'Actor' || role === 'Dancer') && work.fileUrl && (
                    fromDrive ? (
                      <iframe src={driveEmbedUrl(work.fileUrl)}
                        style={{ width:'100%', height:'220px', border:'none', borderRadius:'10px', marginTop:'10px' }}
                        allow="autoplay" title={work.title}/>
                    ) : (
                      <video controls src={work.fileUrl} style={{ width:'100%', borderRadius:'10px', marginTop:'10px', maxHeight:'220px' }}/>
                    )
                  )}

                  {/* Actor / Dancer — missing fileUrl warning */}
                  {(role === 'Actor' || role === 'Dancer') && !work.fileUrl && (
                    <div style={{ marginTop:'10px', background:'#fff3cd', border:'1px solid #ffc107', borderRadius:'8px', padding:'8px 12px', fontSize:'12px', color:'#856404' }}>
              ...
              <div key={req.firebaseKey} style={{
                ...
                {req.status === 'pending' && (
                  <div style={{ display:'flex', gap:'10px' }}>
                    <button onClick={() => updateTalentRequest(req,'approved')} style={greenBtn}>✅ Accept</button>
                    <button onClick={() => updateTalentRequest(req,'declined')} style={redBtn}>❌ Decline</button>
                  </div>
                )}
                {req.status === 'approved' && (
