        ...
        {display.length === 0 ? (
          ...
          if (item._type === 'follow') return (
            <div key={item.firebaseKey || idx}
              style={{ ...nCard, borderLeft:'4px solid #fd79a8', background: item.read ? '#f9f9f9' : '#fff0f7' }}
            ...
            >
              <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:'10px', color:'#b2bec3', marginTop:'3px' }}>
            ...
            <div key={item.firebaseKey || idx}
              ...
              <div style={{ display:'flex', alignItems:'flex-start', gap:'10px' }}>
                ...
                <div style={{ flex:1, fontSize:'13px', lineHeight:1.5 }}>
                  {isWriter ? (
                    ...
                    <><strong>"{item.storyTitle}"</strong> — <strong>{getTypeText(item)}</strong> request is <strong style={{ color:sColor(item.status) }}>{item.status}</strong>!</>
                  )}
                </div>
              </div>
              ...
              {!isWriter && item.status==='approved' && (
                <button
                  ...
                  style={{ ...appBtn, background:'#2d3436', marginTop:'8px', fontSize:'11px' }}>
                  📖 View Story
                </button>
              )}
            </div>
...
const avSmall    = { width:'36px', height:'36px', borderRadius:'50%', objectFit:'cover', border:'1px solid #eee', flexShrink:0 };
...
const profileBtn = { marginTop:'8px', padding:'5px 12px', background:'#4834d4', color:'#fff', border:'none', borderRadius:'8px', cursor:'pointer', fontSize:'11px', fontWeight:'700' };
